...
class Ghost extends MovingObject {
  constructor(options) {
    ...
    this.cookieRadius = 20;
    this.chocolateChipRadius = 1.25;
    this.animate = true;
    this.numSteps = 0;
    this.pupilOffsetLeft =  [[-5, -5], [-8, -2], [-11, -5], [-8, -8]];
    this.pupilOffsetRight =  [[11, -5], [8, -2], [5, -5], [8, -8]];
    this.isEdible = false;
    this.edibleCounter = 0;
    this.edibleCounterLimit = 600;
    ...
    this.renderEatenCounterLimit = 500;
  ...
  renderAtBase() {
    if (this.renderEatenCounter === this.renderEatenCounterLimit) {
      ...
      if (this.name === "blinky") {
        this.pos = [11.5 * Loader.BLOCK_SIZE, 14.5 * Loader.BLOCK_SIZE];
      } else if (this.name === "pinky") {
        this.pos = [13.167 * Loader.BLOCK_SIZE, 14.5 * Loader.BLOCK_SIZE];
      } else if (this.name === "inky") {
        this.pos = [14.837 * Loader.BLOCK_SIZE, 14.5 * Loader.BLOCK_SIZE];
      } else {
        this.pos = [16.5 * Loader.BLOCK_SIZE, 14.5 * Loader.BLOCK_SIZE];
      }
  ...
  renderChocolateChips() {
    ...
    this.ctx.beginPath();
    this.ctx.arc(
      this.pos[0]  + chocChipPos[0][0],
      this.pos[1]  + chocChipPos[0][1],
      this.chocolateChipRadius,
      0, 
      2*Math.PI 
    );
    this.ctx.moveTo(
      this.pos[0]  + chocChipPos[1][0],
      this.pos[1]  + chocChipPos[1][1]
    );
    this.ctx.arc(
      this.pos[0]  + chocChipPos[1][0],
      this.pos[1]  + chocChipPos[1][1],
    ...
    this.ctx.moveTo(
      this.pos[0]  + chocChipPos[2][0],
      this.pos[1]  + chocChipPos[2][1]
    );
    this.ctx.arc(
      this.pos[0]  + chocChipPos[2][0],
      this.pos[1]  + chocChipPos[2][1],
    ...
    this.ctx.moveTo(
      this.pos[0]  + chocChipPos[3][0],
      this.pos[1]  + chocChipPos[3][1]
    );
    this.ctx.arc(
      this.pos[0]  + chocChipPos[3][0],
      this.pos[1]  + chocChipPos[3][1],
  ...
  renderPupils(state) {
    ...
    this.ctx.beginPath();
    this.ctx.arc(
      this.pos[0] + this.pupilOffsetLeft[state][0], 
      this.pos[1] + this.pupilOffsetLeft[state][1],
      this.pupilRadius,
      0, 
      2*Math.PI, 
    );
    ...
    this.ctx.beginPath();
    this.ctx.arc(
      this.pos[0] + this.pupilOffsetRight[state][0], 
      this.pos[1] + this.pupilOffsetRight[state][1],
      this.pupilRadius,
      0, 
      2*Math.PI, 
    );
  ...
  render() {
    ...
    if (this.renderEaten) {
      ...
      if (this.edibleCounter === this.edibleCounterLimit) {
        ...
        this.toggleRender ? this.renderGhost(state) : this.renderChocolateChipCookie();
